...
const ABSENCE_LABEL: Record<AbsenceCode, string> = {
  H: "Holiday", V: "Vacation", S: "Sick Day", C: "Com-time", B: "Business trip", R: "Bereavement", "H/D": "Half Day",
};
...
function CellInput({
  ...
  return (
    <div className="flex flex-col items-center gap-0.5">
      ...
      {/* Hours input (shown only when no absence code) */}
      {!code && (
        <input
          type="number"
          min="0"
...
export function TimesheetGrid({
  ...
  for (const e of entries) {
    ...
    if (!initialRows.find((r) => rowKey(r) === key)) {
      initialRows.push({ projectId: e.projectId, phase: e.phase });
    }
  }
  ...
  const availableToAdd = projects.filter(
    (p) => !activeRows.find((r) => r.projectId === p.id && r.phase === (rowPhases[`${p.id}|`] ?? ""))
  );

  function addRow(projectId: string) {
    if (!projectId) return;
    // Default phase empty; user can pick
  ...
  function removeRow(key: string) {
    ...
    if (!row) return;
    setActiveRows((prev) => prev.filter((r) => rowKey(r) !== key));
  ...
  async function handleSave() {
    ...
    setError("");
    try {
      const entriesToSave = localEntries.filter((e) =>
        activeRows.find((r) => r.projectId === e.projectId && r.phase === e.phase)
      );
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const submitRes = await fetch(`/api/timesheets/${timesheetId}/submit`, { method: "POST" });
      ...
      <div className="overflow-x-auto rounded-lg border border-neutral-200">
        <table className="w-full text-sm">
          ...
          <tbody>
            {activeRows.length === 0 && (
              <tr>
                <td colSpan={12} className="text-center py-10 text-neutral-400 text-sm">
            ...
            {activeRows.map((row, idx) => {
              ...
              const rowTotal = getRowTotal(row.projectId, row.phase);
              const hasData = weekDays.some((day) => {
                const cell = getCellValue(row.projectId, row.phase, day);
                return (cell?.hours ?? 0) > 0 || cell?.absenceCode;
              });
